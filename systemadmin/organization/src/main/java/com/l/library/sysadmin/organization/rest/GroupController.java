package com.l.library.sysadmin.organization.rest;


import com.l.library.manangement.common.core.entity.vo.Result;
import com.l.library.sysadmin.organization.entity.form.GroupForm;
import com.l.library.sysadmin.organization.entity.form.GroupQueryForm;
...
public class GroupController {
    ...
    public Result add(@Valid @RequestBody GroupForm groupForm) {
        log.debug("name:{}", groupForm.getName());
    ...
    @ApiOperation(value = "修改用户组", notes = "根据url的id指定修改对象")
    @ApiImplicitParams({
    ...
    @PutMapping(value = "/{id}")
    public Result update(@PathVariable("id") String id, @Valid @RequestBody GroupForm groupForm) {
        Group group = groupForm.toPo(id, Group.class);
        return Result.success(groupService.update(group));
    }

    @ApiOperation(value = "获取用户组", notes = "根据url的id获取指定用户组")
    @ApiImplicitParam(name = "id", value = "用户组id", required = true, dataType = "long")
    @GetMapping("/{id}")
